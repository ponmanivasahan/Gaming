...
function determineWinner({ player, enemy, timerId }) {
  ...
  const roundNum = window.gameState.rounds.current;
  ...
  if (player.health > enemy.health) {
    ...
    if (window.gameState.soundSystem) window.gameState.soundSystem.play('victory');
  ...
  if (matchOver) {
    ...
    if (window.gameState.rounds.player > window.gameState.rounds.enemy) {
      document.querySelector('#displayTextMsg').innerHTML =
        `${player1Name} Wins!<br><span style="font-size:0.38em;color:#FFD700;">Best of 3 Champion 🏆</span>`;
      const p1W = parseInt(localStorage.getItem('fighterWinsP1')) || 0;
      localStorage.setItem('fighterWinsP1', (p1W + 1).toString());
      if (window.gameState.aiEnabled && typeof launchConfetti === 'function') launchConfetti();
    } else if (window.gameState.rounds.enemy > window.gameState.rounds.player) {
      document.querySelector('#displayTextMsg').innerHTML =
        `${player2Name} Wins!<br><span style="font-size:0.38em;color:#FFD700;">Best of 3 Champion 🏆</span>`;
    ...
    let msg;
    if (roundWinner === 'player')  msg = `${player1Name} wins Round ${roundNum}!`;
    else if (roundWinner === 'enemy') msg = `${player2Name} wins Round ${roundNum}!`;
    else                           msg = `Round ${roundNum} — Tie!`;
...
function launchConfetti() {
  const COLORS = [
    '#ff4444','#ff8800','#ffdd00','#44ff66','#44aaff',
    '#aa44ff','#ff44cc','#ffffff','#ff6699','#00ffcc'
  ];
